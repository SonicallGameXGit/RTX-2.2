...
float random(vec2 uv) { 
	return fract(sin(dot(uv * 10.0 + time * 10.0, vec2(12.9898, 4.1414))) * 43758.5453);
}
...
HitInfo checkBox(Ray ray, Box box) {
    ...
    return HitInfo(tN >= 0.0, tN, tF, step(t1.yzx, t1.xyz) * step(t1.zxy, t1.xyz) * -sign(ray.direction), box.material);
}

HitInfo raycast(Ray ray) {
    HitInfo nearestInfo = HitInfo(false, 1000000.0, 0.0, vec3(0.0), Material(vec3(0.0), 0.0, false));

    Sphere sphere = Sphere(vec3(0.0, 0.0, 6.0), 1.0, Material(vec3(0.9, 0.0, 0.0), 0.7, false));
    ...
    Box box = Box(vec3(-10.0, -1.2, -10.0), vec3(20.0, 0.2, 20.0), Material(vec3(0.8), 0.7, false));
...
vec3 raytrace(Ray ray, float randomOffset) {
    ...
    for(int i = 0; i < 64; i++) {
        ...
        diffuse = normalize(diffuse * dot(diffuse, raycastInfo.normal));

        ray.direction = normalize(mix(reflect(ray.direction, raycastInfo.normal), diffuse, raycastInfo.material.diffuse));
    }
